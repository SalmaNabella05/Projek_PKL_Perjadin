          ...
          <div class="container">
              ...
              <div class="widget widget-table action-table">
                  <div class="widget-header"> <i class="icon-th-list"></i>
                      <h3>Edit Data Rencana Perjadin</h3>
                  </div> <!-- /widget-header -->
                  <div class="widget-content">
                      ...
                      <?php endif; ?>
                      <form action="<?= base_url('CSupervisor/edit_rencana/' . $rencana['id_rencana']) ?>" method="post" enctype="multipart/form-data" style="margin-left: 10px; padding: 10px;">
...
================================================== -->
  ...
  <script type="text/javascript">
      $(document).ready(function() {
          ...
          $('#output').change(function() {
              var id = $(this).val();
              $.ajax({
                  url: "<?php echo base_url(); ?>CSupervisor/select_kegiatan_output",
                  method: "POST",
                  data: {
                      id: id
                  },
  ...
  <script>
      function setVolume() {
          ...
          $.ajax({
              method: "post",
              url: "<?php echo base_url(); ?>CSupervisor/select_kegiatan_id", // url nya arahin ke function durasi
              data: {
                  v1: v1
              },
              async: false,
  ...
  <script type="text/javascript">
      $('#tanggal_pulang').change(function() {
          ...
          if (tanggal_pulang < tanggal_berangkat) {
              ...
              $.ajax({
                  method: "post",
                  url: "<?php echo base_url(); ?>CSupervisor/hitung_volume_rencana", // url nya arahin ke function durasi
                  data: {
  ...
  <script type="text/javascript">
      $(document).ready(function() {
          ...
          setInterval(() => {
              $.ajax({
                  url: "<?php echo base_url(); ?>CSupervisor/notif",
                  method: "POST",
                  data: {
  ...
  <script type="text/javascript">
      $(document).ready(function() {
          ...
          setInterval(() => {
              $.ajax({
                  url: "<?php echo base_url(); ?>CSupervisor/notif1",
                  method: "POST",
                  data: {
                  ...
                  success: function(data) {
                      ...
                      for (i = 0; i < data.length; i++) {
                          if (data[i].notif == 1) {
                              html += '<li><a href="<?= base_url() ?>CSupervisor/klikNotif/' + data[i].id_rencana + '"><span style="font-size: 10pt; font-style: bold;">' + data[i].nama + '</span><br><span style="font-size: 8pt;">Mengajukan Rencana Perjadin Baru</span></a></li>';
                          }
                          if (data[i].notif == 2) {
                              html += '<li><a href="<?= base_url() ?>CSupervisor/klikNotif/' + data[i].id_rencana + '"><span style="font-size: 10pt; font-style: bold;">' + data[i].nama + '</span><br><span style="font-size: 8pt;">Merubah Rencana Perjadin</span></a></li>';
                          }
                      }
                      $('#list').html(html);
                  }
